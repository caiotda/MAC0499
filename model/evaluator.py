...
def evaluate(model, ds_test, dev):
    """
    ...
    with torch.no_grad():
        for idx, sample in enumerate(ds_test):
                ...
                input_tensor = sample["input_ids"].squeeze().to(dev, dtype = torch.long)
                att_mask = sample["attention_mask"].to(dev, dtype = torch.long)
                target = sample["targets"].to(dev, dtype = torch.long)

                out = model(input_tensor, att_mask, labels=target)
                logits = out['logits']
                loss = out['loss']
                ...
                if (idx % debug_ammount == 0 and idx != 0):
                    prog = "{:.2f}".format(100*idx/len(ds_test))
